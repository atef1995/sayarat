...
export const useListingCreation = ({
  ...
  /**
   * Step 4: Submit the listing using the actual listing service with retry logic
   */
  const submitListing = useCallback(async (): Promise<boolean> => {
    ...
    let attempt = 0;
    const maxAttempts = 2; // Allow one automatic retry

    while (attempt < maxAttempts) {
      attempt++;

      try {
        console.log(`🚀 Listing submission attempt ${attempt}/${maxAttempts}`);
        console.log(
          "Form data keys:",
        ...
        logSubmissionProgress(`Attempt ${attempt}`, {
        ...
        // If not successful and we have more attempts, continue to retry
        ...
        throw createError(
          ListingErrorType.SUBMISSION_FAILED,
          "فشل في إنشاء الإعلان بعد عدة محاولات"
        );
      } catch (error) {
        console.error(
          `❌ Listing submission error on attempt ${attempt}:`,
          error
        );
        ...
        if (isNetworkError && attempt < maxAttempts) {
          console.log(
            `🔄 Network error detected, will retry. Attempt ${attempt}/${maxAttempts}`
          );
